...
var _gaq = _gaq || [];
_gaq.push(['_setAccount', 'UA-4976075-1']);
_gaq.push(['_setDomainName', '.hotel24h.com']);
...
$.IsValid_Email = function(emailAddress) {
var pattern = new RegExp(/^(("[\w-+\s]+")|([\w-+]+(?:\.[\w-+]+)*)|("[\w-+\s]+")([\w-+]+(?:\.[\w-+]+)*))(@((?:[\w-+]+\.)*\w[\w-+]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i);
return pattern.test(emailAddress);
};
$.IsValid_Phone = function(phone) {
var pattern = new RegExp(/^([0-9\(\)\/\+ \-]*)$/);
return pattern.test(phone);
};
$.GetDialogPosition_X = function(objThis, x) {
return objThis.offset().left + x;
}
$.GetDialogPosition_Y = function(objThis, y) {
return objThis.offset().top - $(document).scrollTop() + y;
}
$.SafeData = function (_data) {
if (_data && _data.replace)
return $.trim(_data).replace(/[\'|;|\[|\]|\||\~|\{|\}]/g, "");
else
return _data;
}
$.SafeXmlData = function (_data) {
if (_data && _data.replace)
return $.trim(_data).replace(/\'/g, "\\'");
else
return _data;
}
$.GetNextDate = function(date, incrementDay) {
...
//var TotalAmount = parseInt($("input.txtBaseRateAfterTaxVnd_SubTotal").val());
//$("span.TotalVND").text($.AddDots(TotalAmount));
...
// DisplayGlobalMessage - start
// state: highlight, error, active, default
$.DisplayGlobalMessage = function (msg, state) {
if ($("div.divGlobalMessage").length > 0) {
$("div.divGlobalMessage").remove();
}
if (!state) state = "highlight";
var html = '<div class="divGlobalMessage ui-widget-content ui-corner-all hide ui-state-' + state + '" style="z-index:999999;position:fixed;top:2px;right:2px;opacity:.9;"><div class="divMessage" style="float:left;margin:10px 20px 10px 20px;font-weight:bold;font-size:1.3em;"></div><div class="divClose" style="float:right;margin:5px 5px 0 0;"><a class="lnkClose" href="#"><img src="/M/Mobile_v2/img/icons/xbox.png"></a></div><div class="clear"></div></div>';
$("body").append(html);
$("div.divMessage", $("div.divGlobalMessage")).html(msg);
$("div.divGlobalMessage").slideDown();
setTimeout(function () {
$("div.divGlobalMessage").remove();
}, 60000); //1000=1sec
}
...
try {
window.onerror = function (msg, url2, linenumber) {
if (errorSent < 3) {
errorSent++;
$.ajax({
type: "POST",
url: "/M/Mobile_v2/WebServices/Errors.asmx/LogError1",
data: "{'PageUrl': '" + $.SafeData(url2) + "', 'ErrorMessage': '" + $.SafeData(msg) + "', 'LineNumber': '" + $.SafeData(linenumber) + "', 'Referrer': '" + $.SafeData(document.referrer) + "'}",
...
var jqxhrResponseText = 'Error';
if (jqxhr.status == 0) {
jqxhrResponseText = 'Not connected. Please verify your network connection.';
} else if (jqxhr.status == 404) {
jqxhrResponseText = 'The requested page not found 404.';
} else if (jqxhr.status == 500) {
jqxhrResponseText = 'Internal Server Error 500.';
if (jqxhr.responseText != undefined) {
if (jqxhr.responseText.length < 500) {
jqxhrResponseText += ' Error: ' + $.SafeData(jqxhr.responseText) + '.';
} else {
jqxhrResponseText += ' Error: ' + escape($.SafeData(jqxhr.responseText)) + '.';
}
}
} else if (exception == 'parsererror') {
jqxhrResponseText = 'Requested JSON parse failed.';
} else if (exception == 'timeout') {
jqxhrResponseText = 'Time out error.';
} else if (exception == 'abort') {
jqxhrResponseText = 'Ajax request aborted.';
} else {
jqxhrResponseText = jqxhr.responseText;
}
try {
$.ajax({
type: "POST",
url: "/M/Mobile_v2/WebServices/Errors.asmx/LogError2",
data: "{'PageUrl': '" + $.SafeData(settings.url) + "', 'ErrorMessage': '" + $.SafeData(jqxhrResponseText) + "', 'LineNumber': '-1', 'Referrer': '" + $.SafeData(document.referrer) + "'}",
contentType: "application/json; charset=utf-8",
dataType: "json"
});
} catch (e) { }
